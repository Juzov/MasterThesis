	...
	for (i = 0; i < *nr; i++){
		for (j = 0; j < *nc; j++) {
			...
			dispersion += subspace_weights[index]
					* log(1 + pow(x[j * (*nr) + i] - o_prototype[index], 2));
		}
	}
		...
		double *o_prototype) // Numeric prototype matrix (k*nc)
{
	...
	double *cluster_distances, *new_prototypes;


	inner_distance = (double *) malloc(sizeof(double));
	cluster_distances = (double *) calloc(*k * *nc, sizeof(double));
	new_prototypes = (double *) calloc(*k * *nc, sizeof(double));
	...
	for (i = 0; i < *nr; i++) {
		for (j = 0; j < *nc; j++){
			*inner_distance = pow(1 + pow(x[j * (*nr) + i] - o_prototype[j * (*k) + partition[i]], 2), -1);
			...
			new_prototypes[j * (*k) + partition[i]] += *inner_distance * x[j * (*nr) + i];
		}
	}
	for (l = 0; l < *k; l++) {
		for (j = 0; j < *nc; j++){
			o_prototype[j * (*k) + l] = new_prototypes[j * (*k) + l] / cluster_distances[j * (*k) + l];
		}
	}
		...
		double *subspace_weights) // Weights for variable/cluster (k*nc)
{
	...
	for (i = 0; i < *nr; i++) {
		...
		for (j = 0; j < *nc; j++) {
			index = j * (*k) + partition[i];
			subspace_weights[index] += log( 1 + pow(
	...
	for (l = 0; l < (*k); l++) {
		for (j = 0; j < *nc; j++) {
			index = j * (*k) + l;
			subspace_weights[index] /= (double) no_clusters[l];
		}
	}
